...
const game = {
  ...
  bossSpawnTimer: 15, // Boss 存活时每 15 秒在其附近刷一个小怪
  last: Date.now(),
...
function spawnCore() { return { x: 120 + Math.random() * (WORLD.width - 240), y: 100 + Math.random() * (WORLD.height - 200), live: true, timer: 0 }; }
const ITEM_TYPES = [
...
function spawnItem(x, y) {
  const r = Math.random(); // 血包 40% 权重，其余三种各 20%
  const t = r < .4 ? ITEM_TYPES[0] : ITEM_TYPES[1 + Math.floor((r - .4) / .2)];
...
function aiInput(bot, now, dt) {
  ...
  if (boss) { // 协同：软束缚——太远柔和拉回、太近柔和推开，避免边缘抽搐
    const dB = distance(bot, boss);
    const minR = 140, maxR = 300;
    let pullX = 0, pullY = 0;
    if (dB > maxR) { const k = Math.min(1, (dB - maxR) / 160); pullX = (boss.x - bot.x) / dB * k; pullY = (boss.y - bot.y) / dB * k; }
    else if (dB < minR) { const k = Math.min(1, (minR - dB) / 120); pullX = (bot.x - boss.x) / dB * k; pullY = (bot.y - boss.y) / dB * k; }
    x = x + pullX * 360; y = y + pullY * 360;
...
function tick() {
  ...
  if (game.phase === 'playing') {
    ...
    for (const actor of [game.player, ...game.bots]) for (const core of game.cores) if (core.live && actor.respawn <= 0 && distance(actor, core) < actor.pickup) { core.live = false; core.timer = 5.5; if (actor.kind === 'player') { game.score++; addXp(10); effect('+10 经验', actor.x, actor.y, '#9bff6a'); } }
    ...
    game.bullets = game.bullets.filter(b => { b.x += b.dx * dt; b.y += b.dy * dt; b.life -= dt; const targets = b.team === 'player' ? game.bots : [game.player]; const target = targets.find(t => t.respawn <= 0 && (t.invuln || 0) <= 0 && distance(b, t) < (b.fire ? 26 : 24)); if (target) { target.hp -= b.damage * (1 - target.resist); effect(`-${Math.round(b.damage)}`, target.x, target.y - 25, '#ff8d8d'); b.life = 0; if (target.hp <= 0) { if (target.kind === 'bot') { target.dead = true; const xp = target.boss ? 15 : target.type === 'brute' ? 4 : target.type === 'shooter' ? 3 : 2; game.score += target.boss ? 8 : target.type === 'brute' ? 3 : 2; addXp(xp); effect(target.boss ? '首领击败！' : `+${xp} 经验`, target.x, target.y, target.boss ? '#ffffff' : '#9bff6a'); if (target.boss || Math.random() < .25) spawnItem(target.x, target.y); } } } return b.life > 0 && b.x > 0 && b.x < WORLD.width && b.y > 0 && b.y < WORLD.height; });
    game.bots = game.bots.filter(bot => !bot.dead);
    const bossAlive = game.bots.find(x => x.boss);
    if (bossAlive && game.phase === 'playing') { // Boss 存活时每 15 秒在附近刷一个小怪
      game.bossSpawnTimer -= dt;
      if (game.bossSpawnTimer <= 0) {
        game.bossSpawnTimer = 15;
        game.bots.push(makeEnemy(Math.random() < .6 ? 'scout' : 'shooter', Math.random() * 6, bossAlive));
        effect('小怪增援', bossAlive.x, bossAlive.y - 40, '#bcecff');
      }
    } else {
      game.bossSpawnTimer = 15;
    }
